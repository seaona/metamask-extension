...
describe('permission background API methods', () => {
  ...
  describe('requestAccountsPermissionWithId', () => {
    it('request an accounts permission and returns the request id', async () => {
      const permissionController = {
        requestPermissions: jest.fn().mockImplementationOnce(() => {
          return Promise.resolve([null, { id: 'arbitraryId' }]);
        }),
      };
      ...
      await new Promise((resolve) => {
        getPermissionBackgroundApiMethods(
          permissionController,
        ).requestAccountsPermissionWithId('foo.com', (_err, id) => {
          expect(permissionController.requestPermissions).toHaveBeenCalledTimes(
            1,
          );
